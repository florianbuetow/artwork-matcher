...
1. **End-to-End Performance Tests**
   - Full pipeline with all services running
   - Real HTTP communication between services
   - Realistic latency measurements
   - Location: `tests/e2e/performance/` or `tools/performance/`

2. **E2E Test Scenarios Needed:**
   - Single image identification (happy path)
   - Batch identification (multiple images)
   - No-match scenario (image not in database)
   - Concurrent users simulation
   - Sustained load over time

3. **Infrastructure:**
   - Docker Compose test configuration
   - Test data fixtures (real images)
   - Automated service startup/teardown

### Medium Priority

4. **Memory Profiling**
   - Peak memory usage per service
   - Memory growth under load
   - GPU memory tracking (embeddings)

5. **Cold Start Testing**
   - Model loading time (embeddings)
   - Index loading time (search)
   - First-request latency vs. warm requests
...
7. **Comparative Benchmarks**
   - Different model sizes (embeddings)
   - Different index types (search)
   - Different ORB configurations (geometric)

8. **Historical Tracking**
   - Store results in database/file
...
| Service | Testing Level | What's Measured | Status |
|---------|---------------|-----------------|--------|
| Embeddings | Isolation | DINOv2 inference + preprocessing | Complete |
| Search | Isolation | FAISS search + ranking | Complete |
| Geometric | Isolation | ORB + matching + RANSAC | Complete |
| Gateway | Partial Isolation | Routing + orchestration (backends mocked) | Complete |
| **Full System** | **End-to-End** | **Complete pipeline latency** | **TODO** |

The current test suite provides excellent coverage of individual service performance but lacks visibility into real-world system behavior. End-to-end tests are the critical missing piece for production readiness assessment.
